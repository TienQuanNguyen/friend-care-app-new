...
export const MoodJournal = () => {
  ...
  const scrollToEntry = (entryId: string) => {
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        const el = document.getElementById(`mood-entry-${entryId}`);
        if (el) {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-amber-400 hover:text-amber-600 transition-colors p-1 shrink-0 opacity-0 group-hover:opacity-100"
                      title="Bỏ ghim"
                    >
                ...
                {displayedEntries.map(entry => {
                  ...
                  return (
                    <Card
                      ...
                      className={`bg-white border shadow-sm rounded-2xl p-5 hover:shadow-md transition-all ${
                        highlightedEntryId === entry.id
                          ? 'border-brand ring-2 ring-brand/30 animate-pulse'
                          : 'border-canvas-cool'
                      }`}
                    >
