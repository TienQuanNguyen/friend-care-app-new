...
export const adviceService = {
  async getAdvice(request: AdviceRequest): Promise<string> {
    try {
      ...
      if (!response.ok) {
        console.warn(`[adviceService] API call failed with status ${response.status}. Using fallback.`);
      ...
      console.warn('[adviceService] Invalid API response format. Using fallback.');
      return this.getFallbackAdvice(request);
    } catch (error: any) {
      if (error.name === 'AbortError') {
        console.warn('[adviceService] API call timed out. Using fallback.');
      } else {
        console.warn('[adviceService] API call error:', error.message || error);
      }
      return this.getFallbackAdvice(request);
    }
  },
